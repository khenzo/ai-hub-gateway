...
flowchart TD
    subgraph APL["AZURE PLATFORM LANDING ZONE"]
        direction LR  // Ensures left-to-right flow
        EID["Entra ID\n(Identity)"] --- HUB["Hub VNet\n(Network)"] --- LA1["Log Analytics\n(Monitoring)"] --- AZP["Azure Policy\n(Governance)"]
    end
...
flowchart TD
    subgraph L1["Layer 1: Perimeter"]
        direction LR
        AF["Azure Firewall\ntraffic inspection"] --- DD["DDoS Protection"] --- NSG["NSGs\nnetwork segmentation"]
    end

    subgraph L2["Layer 2: Gateway"]
        direction LR
        OA["OAuth 2.0 / JWT validation"] --- RL["Rate limiting and quotas"] --- CF["Content filtering"] --- PII["PII detection"]
    end

    subgraph L3["Layer 3: Backend"]
        direction LR
        PE["Private endpoints"] --- MI["Managed identities"] --- ENC["Encryption at rest/in transit"]
    end

    subgraph L4["Layer 4: Observability"]
        direction LR
        AL["Audit logging"] --- SA["Security alerts"] --- CM["Compliance monitoring"]
    end
