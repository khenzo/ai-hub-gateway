...
description: "Authentication and authorization validation with Microsoft Entra ID for AI Hub Gateway"
---
...
Client Application
       │
       ├─ 1. Authenticate & request token ────► Entra ID
       │                                          │
       │◄──────── 2. Return JWT token ───────────┤
       │
       ├─ 3. API Request with Bearer token + sub key ────► APIM Gateway
                                                         │
                                                         ├─ 4. Validate subscription key
                                                         ├─ 5. Check entra-auth named value
                                                         ├─ 6. Validate JWT using OpenID config ────► Entra ID
                                                         ├─ 7. Validate audience, issuer, client-id
                                                         │
                                              ┌──────────┴──────────┐
                                              │   JWT Valid?        │
                                              └──────────┬──────────┘
...
Similar to standard approach, but with variables set in policy:
- Set `tenant-id`, `audience`, `client-id` as variables
- Use dynamic OpenID configuration
- Validate dynamic claims
...
- Validate audience claim in token
  ...
  <Accordion title="Variable Resolution Issues (Custom Approach)">
...
<CardGroup cols={3}>
  <Card title="Full Deployment" icon="server" href="/guides/citadel-hub/deployment/full-deployment">
    Production deployment with Entra ID
  </Card>
