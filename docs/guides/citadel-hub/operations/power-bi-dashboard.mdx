...
description: "Usage analytics and monitoring with Power BI for AI Hub Gateway"
---
...
- Insert initial data into the model-pricing container (sample data for the model-pricing container can be found in the <a href="https://github.com/Azure-Samples/ai-hub-gateway-solution-accelerator/blob/citadel-v1/src/usage-reports/model-pricing.json" target="_blank">model-pricing.json</a>).

![Insert Data](/images/cosmos-db-model-pricing.png)

<Note>
Pricing in the sample file is based on the public Azure pricing for East US region (which in many cases similar to other regions). You can review the prices on Azure docs related to the service being used and update the `model-pricing` accordingly.
</Note>
...
Now you can open the <a href="https://github.com/Azure-Samples/ai-hub-gateway-solution-accelerator/blob/citadel-v1/src/usage-reports/AI-Hub-Gateway-Usage-Report-v1-5-Incremetal.pbix" target="_blank">AI-Hub-Gateway-Usage-Report-v1-5-Incremetal.pbix</a> file in the Power BI Desktop application.
...
| **Cost Analysis** | Estimated costs by model and time period |
| **Model Breakdown** | Usage distribution across models |
| **Application Insights** | Usage by application/consumer |
| **Geographic Distribution** | Usage by region |
| **Throttling Events** | 429 errors and retry patterns |
...
<Note>
You can extend data model to include additional metadata that can be captured from the API request. Keep in mind that this will require updates to usage collection policy fragments (like ai-usage, openai-usage and openai-usage-streaming).
</Note>
...
- **Power BI Service embed** - For internal applications
- **Export to PDF/PNG** - For static reports
...
- Ensure Cosmos DB is in online state
- Validate account keys
</Accordion>
  <Accordion title="No Data Showing">
- Verify data exists in Cosmos DB containers
- Check query filters in Power BI
- Ensure date range includes data
- Validate model-pricing data exists
</Accordion>
  <Accordion title="Refresh Failures">
- Check gateway connectivity (if using on-prem gateway)
- Verify credentials are up to date
- Review refresh schedule settings
- Check Cosmos DB throttling
</Accordion>
</AccordionGroup>

## Best Practices

1. **Schedule regular refreshes** - Keep data current (hourly recommended)
2. **Use filters** - Enable users to focus on relevant data
3. **Document metrics** - Explain how costs are calculated
4. **Secure sensitive data** - Use row-level security if needed
5. **Version control** - Save PBIX files with version numbers
6. **Train users** - Provide dashboard navigation guidance
...
<CardGroup cols={3}>
  <Card title="Usage Analytics" icon="chart-pie" href="/guides/citadel-hub/operations/usage-analytics">
