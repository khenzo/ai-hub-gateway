...
**Layer 4 — Security Fabric** is the **unified protection layer** of the SOF1A 2.0 AI Governance Platform. It provides **real-time defense against AI-specific threats** and integrates across all governance layers. By weaving together **Microsoft Defender**, **Microsoft Purview**, and **Microsoft Entra**, this layer delivers unified protection that closes the loop from policy definition to runtime defense.
...
graph TB
    ...
    PUR["📚 Purview\nData Governance"]
    ENT["🔐 Entra\nIdentity"]
    USO["Unified Security Operations\nCross-signal correlation · Coordinated response\nSingle pane of glass · Automated remediation"]
...
<Note>
**Data Foundation: Governance by Construction via Purview Column Masks**

Purview sensitivity labels assigned to Unity Catalog columns propagate through the APIM data
access policy to AI agent responses — column masks are enforced at the API boundary, not just
at the warehouse. This means AI agents receive pre-filtered data without requiring application-
level redaction logic. See [Security & Identity Patterns](../data-foundation/security-identity-patterns)
for the Entra Managed Identity chain and column mask propagation design.
</Note>
