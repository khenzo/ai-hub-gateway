...
graph TB
    subgraph HubVNet["Enterprise Hub Virtual Network"]
        subgraph CGH["Citadel Governance Hub"]
            GW["AI Gateway (APIM)<br/>Routing · Rate Limit · Auth"]
            PE["Policy Engine (Content Safety)<br/>Safety Checks"]
            OB["Observability<br/>Log Analytics · App Insights · Cosmos DB"]
        end
        subgraph SS["Shared Services"]
            AD["Azure AD DS"]
            DNS["DNS"]
            FW["Azure Firewall (Shared)"]
        end
    end
    subgraph Spoke["Agent Spoke Network"]
        AW["Agent Workloads<br/>🤖 AI Agents"]
    end
...
graph TB
    subgraph HubVNet["Enterprise Hub Virtual Network"]
        subgraph SS["Shared Services"]
            AD["Azure AD DS<br/>Identity · AD Connect"]
            DNS["DNS Servers<br/>Forwarding"]
            FW["Azure Firewall<br/>Traffic Inspect · Threat Intel"]
        end
    end
    subgraph CGH["Citadel Governance Hub (Dedicated Spoke)"]
        GW["AI Gateway (APIM)<br/>Routing · Rate Limit · Auth"]
        PE["Policy Engine (Content Safety)"]
        OB["Observability<br/>Log Analytics · App Insights · Cosmos DB"]
    end
    subgraph AgentSpokes["Agent Spoke Networks"]
        FIN["Finance Spoke<br/>🤖 AI Agents"]
        HR["HR Spoke<br/>🤖 AI Agents"]
        LEG["Legal Spoke<br/>🤖 AI Agents"]
    end
    HubVNet -->|VNet Peering| CGH
    CGH -->|VNet Peering| AgentSpokes
```
...
```mermaid
flowchart TD
    A{Require firewall inspection<br/>of ALL AI traffic?} -->|YES| Spoke["Spoke Network Pattern"]
    A -->|NO| B{Strict compliance<br/>SOC 2 / HIPAA?}
    B -->|YES| Spoke
    B -->|NO| C{Hub VNet has<br/>available address space?}
    C -->|YES| D{Low latency critical?}
    C -->|NO| Spoke
    D -->|YES| Hub["Hub Network Pattern"]
    D -->|NO| Either["Either pattern works"]
```
